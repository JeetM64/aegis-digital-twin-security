...
const styles = `
  ...
  .sidebar-status {
    ...
    font-family: 'JetBrains Mono', monospace;
    font-size: 11px; color: #51cf66;
  }
  .status-dot {
    width: 6px; height: 6px;
    background: #51cf66; border-radius: 50%;
  ...
  .nav-item.danger.active {
    background: linear-gradient(90deg, rgba(255,77,77,0.12), rgba(255,107,107,0.04));
  ...
  .nav-item.danger.active .nav-label { color: #ff4d4d; font-weight: 600; }
  .nav-item:hover .nav-label { color: #b0bec5; }
  .nav-item.danger:hover .nav-label { color: #ff6b6b; }
  .nav-badge {
    margin-left: auto;
  ...
  .user-avatar {
    ...
    background: linear-gradient(135deg, #00e5ff, #0091ea);
    display: flex; align-items: center; justify-content: center;
    font-size: 13px; font-weight: 700; color: #001e3c;
  ...
  .logout-btn {
    ...
    color: #546e7a; cursor: pointer; font-size: 16px;
...
const menuItems = [
  { icon: '⬡',  label: 'Dashboard',           path: '/dashboard',       section: 'OVERVIEW', danger: false },
  { icon: '◈',  label: 'Asset Inventory',     path: '/assets',          section: 'MONITOR',  danger: false },
  { icon: '◎',  label: 'Scan Management',     path: '/scans',           section: 'MONITOR',  danger: false },
  { icon: '◬',  label: 'Vulnerabilities',     path: '/vulnerabilities', section: 'MONITOR',  danger: false },
            ...
            <div key={section}>
              ...
              {menuItems
                ...
                .map((item, idx) => (
                  <div
                    key={item.path}
                    className={`nav-item ${item.danger ? 'danger' : ''} ${isActive(item.path) ? 'active' : ''}`}
