...
from ml.risk_model import predict_risk, INTERNET_PORTS
...
def run_scan(scan_id: int, target: str, mode: str = "fast"):
    ...
    try:
        ...
        _update(scan, total_open_ports=total_ports)
        ...
        for host in hosts:
            ...
            vm = _upsert_vm(ip, hostname, os_name)
            scanned_ips.append((ip, vm.id if vm else None))
...
# ─────────────────────────────────────────────────────────────────────────────
def _process_port(scan_id: int, ip: str, port_data: dict, vm_id: int = None):
    ...
    internet_exp  = port in INTERNET_PORTS
    cve_count     = len(cves)
    cve_ids_str   = ",".join(c["cve_id"] for c in cves[:10])

    # Build remediation suggestion
    remediation = _build_remediation(port, service, exploit_found, misconfig)
    ...
    description = _build_description(
        port, service, version, cves, exploit_found, misconfig
    )
    ...
    if existing:
        ...
        existing.risk_score        = ml_score
        existing.cve_ids           = cve_ids_str
        existing.cve_count         = cve_count
        existing.exploit_available = exploit_found
        existing.is_misconfigured  = misconfig
        existing.internet_exposed  = internet_exp
        existing.remediation       = remediation
        existing.cpe               = cpe
        if vm_id:
            existing.vm_id = vm_id
        db.session.commit()
        return
...
def _update_vm_risk_levels(scanned_ips: list, scan_id: int):
    """Set risk_level on each VM based on worst severity found in this scan."""
    for ip, vm_id in scanned_ips:
        vm = VM.query.filter_by(ip_address=ip).first()
        if not vm:
            continue

        # Query vulns scoped to this vm_id in this scan
        if vm_id:
            vulns = Vulnerability.query.filter_by(
                scan_id=scan_id, vm_id=vm_id
            ).all()
        else:
            vulns = Vulnerability.query.filter_by(scan_id=scan_id).all()
        ...
        if not vulns:
            ...
            if "critical" in severities:   vm.risk_level = "CRITICAL"
            elif "high" in severities:     vm.risk_level = "HIGH"
            elif "medium" in severities:   vm.risk_level = "MEDIUM"
            else:                          vm.risk_level = "LOW"
...
def _update_scan_counts(scan, scan_id: int):
    """Update all summary counts on the scan record."""
    vulns = Vulnerability.query.filter_by(scan_id=scan_id).all()
...
def _upsert_vm(ip: str, hostname: str, os_name: str) -> Optional[VM]:
    try:
        vm = VM.query.filter_by(ip_address=ip).first()
        if vm:
...
def _build_remediation(port: int, service: str, exploit: bool, misconfig: bool) -> str:
    svc = (service or "unknown").lower()
    parts = []

    if exploit:
        parts.append("Patch immediately — known exploit available.")

    remediation_map = {
        "telnet":   "Replace Telnet with SSH. Disable port 23.",
        "ftp":      "Replace FTP with SFTP/SCP. Disable anonymous access.",
        "rdp":      "Restrict RDP behind VPN. Enable Network Level Authentication.",
        "vnc":      "Restrict VNC behind VPN. Use strong password.",
        "smb":      "Patch EternalBlue (MS17-010). Disable SMBv1. Firewall port 445.",
        "mysql":    "Bind MySQL to localhost. Use strong credentials. Disable remote root.",
        "redis":    "Bind Redis to localhost. Enable AUTH password. Firewall port 6379.",
        "mongodb":  "Enable MongoDB authentication. Bind to localhost.",
        "elasticsearch": "Enable Elasticsearch security. Bind to localhost.",
        "snmp":     "Upgrade to SNMPv3. Change default community strings.",
    }

    if svc in remediation_map:
        parts.append(remediation_map[svc])
    elif misconfig:
        parts.append("Review service configuration. Disable default credentials.")
    else:
        parts.append(f"Review {service} configuration. Apply latest security patches.")
